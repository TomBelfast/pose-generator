...
async function findCoolifyServerIP() {
    console.log('🔍 Znajdowanie adresu IP serwera Coolify...\n');
    
    try {
        // 1. Sprawdź IP domeny host.aihub.ovh
        console.log('1️⃣ Sprawdzanie IP domeny host.aihub.ovh...');
        try {
            const { address } = await lookupAsync('host.aihub.ovh');
            console.log(`   - IP: ${address}`);
        } catch (error) {
            console.log(`   - Błąd DNS: ${error.message}`);
        }
        
        // 2. Sprawdź szczegóły aplikacji
        console.log('\n2️⃣ Szczegóły aplikacji...');
        ...
        if (appDetailsResponse.status === 200) {
            ...
            if (app.destination && app.destination.server) {
                console.log('\n3️⃣ Informacje o serwerze Coolify:');
        ...
        // 3. Sprawdź czy aplikacja ma dostępny adres IP
